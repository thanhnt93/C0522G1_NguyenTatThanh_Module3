...
INNER JOIN loai_dich_vu ldv ON ldv.ma_loai_dich_vu = dv.ma_loai_dich_vu
INNER JOIN hop_dong hd ON dv.ma_dich_vu = hd.ma_dich_vu
WHERE YEAR(hd.ngay_lam_hop_dong) = 2020  AND (hd.ma_dich_vu NOT IN(SELECT ma_dich_vu 
																   FROM hop_dong 
...
SELECT MONTH(hd.ngay_lam_hop_dong) thang , COUNT(kh.ma_khach_hang) so_luong_khach_hang
FROM hop_dong hd
