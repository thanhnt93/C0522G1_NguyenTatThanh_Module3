...
SELECT hd.ma_hop_dong, nv.ho_ten ho_ten_nhan_vien, kh.ho_ten ho_ten_khach_hang, kh.so_dien_thoai, dv.ma_dich_vu, dv.ten_dich_vu, sum(hdct.so_luong) so_luong_dich_vu_di_kem, hd.tien_dat_coc
FROM hop_dong hd
...
WHERE (MONTH(hd.ngay_lam_hop_dong) BETWEEN 10 AND 12) AND (YEAR(hd.ngay_lam_hop_dong) = '2020') AND (MONTH(hd.ngay_lam_hop_dong) <> MONTH(hd.ngay_lam_hop_dong) BETWEEN 1 AND 6)
...
INNER JOIN hop_dong_chi_tiet hdct 
	ON dvdk.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
GROUP BY dvdk.ma_dich_vu_di_kem
HAVING so_luong >= ALL (SELECT SUM(hdct.so_luong) 
...
INNER JOIN trinh_do td
	ON nv.ma_trinh_do = td.ma_trinh_do
INNER JOIN vi_tri vt
